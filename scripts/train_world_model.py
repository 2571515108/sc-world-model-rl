...
def main() -> None:
    ...
    use_array_replay = bool(config.get("use_array_replay", False))
    if use_array_replay and mode == "mixed": raise ValueError("array-backed replay currently requires a single data source; use mixed sampler for explicit ratios")
    validation_fraction = float(config.get("validation_fraction", 0.0)); validation_sampler = None; array_sampler = None
    if mode == "mixed":
    ...
    observation_shape = offline.observation_shape if use_array_replay else replay.observation_shape
    model = WorldModel(WorldModelConfig(observation_dim=observation_shape[0], **model_data))
    ...
    trainer = WorldModelTrainer.load(resume, device=str(config.get("device", "auto"))) if resume else WorldModelTrainer(model, learning_rate=float(config["learning_rate"]), max_grad_norm=float(config.get("max_grad_norm", 100.0)), device=str(config.get("device", "auto")), precision=str(config.get("precision", "fp32")))
    with ExperimentLogger(run_dir, config=config, metadata={"random_seed": int(config.get("seed", 7)), "algorithm": "world_model"}) as logger:
        ...
        for update in range(int(config["updates"])):
            burn_in = int(config.get("burn_in_length", 0)); item = trainer.train_array_batch(array_sampler.sample(int(config["batch_size"]), int(config["sequence_length"]), burn_in), burn_in_length=burn_in) if array_sampler is not None else trainer.train_step(sampler.sample(int(config["batch_size"]), int(config["sequence_length"]), burn_in), burn_in_length=burn_in); metrics.append(item)
            ...
            if validation_sampler is not None and (update + 1) % validation_interval == 0:
                validation = trainer.validation_step(validation_sampler.sample(int(config["batch_size"]), int(config["sequence_length"]), burn_in), burn_in_length=burn_in)
                for name, value in validation.items(): logger.log_scalar(name, value, step=update + 1, episode=0, phase="validation")
    evaluation = {} if array_sampler is not None else trainer.open_loop_evaluate(sampler.sample(int(config["batch_size"]), int(config["sequence_length"])))
