...
from .python_sc2_backend import PythonSC2Backend, PythonSC2BackendConfig
...
def build_macro_env(config_path: str):
    ...
    backend = PythonSC2Backend(PythonSC2BackendConfig(
        map_name=str(values.get("map_name", "Abyssal Reef LE")), race=str(values.get("race", "Terran")),
        ...
        timeout_seconds=float(values.get("timeout_seconds", 120.0)), map_width=float(values.get("map_width", 200.0)), map_height=float(values.get("map_height", 200.0)),
    ))
